...
public class AshOfSinCustomAntiSeatEntityEvent {
    ...
    public static void onEntityMountEvent(EntityMountEvent event) {
        if (CustomAntiSeatEntityConfig.ANTI_ON.get()) {
            if (!event.isCanceled() && event.isMounting() && !event.getEntityBeingMounted().level().isClientSide()) {
                ...
                if (antiSeatEntityList.contains(entityResourceLocation.toString()) && !(entityBeingMounted instanceof LivingEntity)) {
                    BlockPos entityPos = entityBeingMounted.blockPosition();
                    entityBeingMounted.level().destroyBlock(entityPos, true);
