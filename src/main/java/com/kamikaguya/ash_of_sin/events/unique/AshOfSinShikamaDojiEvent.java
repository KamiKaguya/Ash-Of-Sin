...
public class AshOfSinShikamaDojiEvent {
    ...
    public static void immunefragility(LivingEvent.LivingTickEvent event) {
        ...
        MobEffect fragility = ForgeRegistries.MOB_EFFECTS.getValue(new ResourceLocation("tensura","fragility"));
        if (holdShikamaDoji(serverPlayer)) {
            if (fragility != null) {
            ...
            if (!alreadyBloodSoaked) {
                ...
                if (!alreadyGrievousBodilyHarm) {
                    ...
                    if (RANDOM.nextFloat() <= 0.15F) {
                        MobEffect insanityEffect = ForgeRegistries.MOB_EFFECTS.getValue(new ResourceLocation("tensura","insanity"));
