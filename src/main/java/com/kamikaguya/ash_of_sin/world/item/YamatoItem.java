...
public class YamatoItem extends WeaponItem{
    ...
    public YamatoItem(Item.Properties build) {
        super(AshOfSinItemTier.YAMATO, 0, -1.85F, build);
        if (AshOfSin.isPhysicalClient()) {
            this.tooltipExpand = new ArrayList<Component>();
            this.tooltipExpand.add(new TextComponent(""));
            this.tooltipExpand.add(new TranslatableComponent("item." + AshOfSin.MODID + ".yamato.tooltip.unique"));
            this.tooltipExpand.add(new TranslatableComponent("item." + AshOfSin.MODID + ".yamato.tooltip"));
        }
    }
